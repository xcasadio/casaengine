...
// 	}
	...
	void beginFrame(uint16_t _width, uint16_t _height, uint16_t _surfaceWidth, uint16_t _surfaceHeight, uint8_t _view)
	{
		m_view = _view;
		m_viewWidth = _width;
...
// 		const int32_t mx = int32_t(float(ImGui::GetIO().MousePos.x)*xscale);
// 		const int32_t my = int32_t(float(ImGui::GetIO().MousePos.y)*yscale);

		IMGUI_beginFrame(_width, _height, _view);
...
void imguiBeginFrame(uint16_t _width, uint16_t _height, uint16_t _surfaceWidth, uint16_t _surfaceHeight, uint8_t _view)
{
	s_imgui.beginFrame(_width, _height, _surfaceWidth, _surfaceHeight, _view);
}

void imguiBeginFrame(uint16_t _width, uint16_t _height, uint8_t _view)
{
	s_imgui.beginFrame(_width, _height, _width, _height, _view);
}

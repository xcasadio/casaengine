...
namespace
{
	...
	struct GLNVGtexture* glnvg__allocTexture(struct GLNVGcontext* gl)
	{
	...
	struct GLNVGtexture* glnvg__findTexture(struct GLNVGcontext* gl, int id)
	{
		int i;
		for (i = 0; i < gl->ntextures; i++)
		{
	...
	int glnvg__deleteTexture(struct GLNVGcontext* gl, int id)
	{
	...
	int nvgRenderCreate(void* _userPtr)
	{
	...
	int nvgRenderDeleteTexture(void* _userPtr, int image)
	{
		struct GLNVGcontext* gl = (struct GLNVGcontext*)_userPtr;
		return glnvg__deleteTexture(gl, image);
	}

	int nvgRenderUpdateTexture(void* _userPtr, int image, int x, int y, int w, int h, const unsigned char* data)
	{
		struct GLNVGcontext* gl = (struct GLNVGcontext*)_userPtr;
	...
	int nvgRenderGetTextureSize(void* _userPtr, int image, int* w, int* h)
	{
		struct GLNVGcontext* gl = (struct GLNVGcontext*)_userPtr;
		struct GLNVGtexture* tex = glnvg__findTexture(gl, image);
	...
	void glnvg__xformToMat3x4(float* m3, float* t)
	{
		m3[ 0] = t[0];
		m3[ 1] = t[1];
		m3[ 2] = 0.0f;
	...
	void glnvg__mat3(float* dst, float* src)
	{
		dst[0] = src[ 0];
		dst[1] = src[ 1];
		dst[2] = src[ 2];
	...
	struct GLNVGfragUniforms* nvg__fragUniformPtr(struct GLNVGcontext* gl, int i)
	{
		return (struct GLNVGfragUniforms*)&gl->uniforms[i];
	}

	void nvgRenderSetUniforms(struct GLNVGcontext* gl, int uniformOffset, int image)
	{
	...
	void nvgRenderViewport(void* _userPtr, float width, float height, float devicePixelRatio)
	{
		struct GLNVGcontext* gl = (struct GLNVGcontext*)_userPtr;
		gl->view[0] = width;
		gl->view[1] = height;
	...
	void fan(uint32_t _start, uint32_t _count)
	{
	...
	void glnvg__fill(struct GLNVGcontext* gl, struct GLNVGcall* call)
	{
	...
	void glnvg__convexFill(struct GLNVGcontext* gl, struct GLNVGcall* call)
	{
		struct GLNVGpath* paths = &gl->paths[call->pathOffset];
		int i, npaths = call->pathCount;
	...
	void glnvg__stroke(struct GLNVGcontext* gl, struct GLNVGcall* call)
	{
		struct GLNVGpath* paths = &gl->paths[call->pathOffset];
	...
	void glnvg__triangles(struct GLNVGcontext* gl, struct GLNVGcall* call)
	{
	...
	uint64_t glnvg_convertBlendFuncFactor(int factor)
	{
	...
	void nvgRenderFlush(void* _userPtr)
	{
		struct GLNVGcontext* gl = (struct GLNVGcontext*)_userPtr;
	...
	int glnvg__maxVertCount(const struct NVGpath* paths, int npaths)
	{
		int i, count = 0;
		for (i = 0; i < npaths; i++)
		{
	...
	int glnvg__maxi(int a, int b) { return a > b ? a : b; }

	struct GLNVGcall* glnvg__allocCall(struct GLNVGcontext* gl)
	{
	...
	int glnvg__allocPaths(struct GLNVGcontext* gl, int n)
	{
		int ret = 0;
	...
	int glnvg__allocFragUniforms(struct GLNVGcontext* gl, int n)
	{
	...
	void glnvg__vset(struct NVGvertex* vtx, float x, float y, float u, float v)
	{
		vtx->x = x;
		vtx->y = y;
		vtx->u = u;
	...
	void nvgRenderTriangles(void* _userPtr, struct NVGpaint* paint, NVGcompositeOperationState compositeOperation, struct NVGscissor* scissor,
	...
	void nvgRenderDelete(void* _userPtr)
	{
		struct GLNVGcontext* gl = (struct GLNVGcontext*)_userPtr;

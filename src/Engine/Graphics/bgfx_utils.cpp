...
{
	return load(s_fileReader, s_allocator, _filePath, _size);
}
...
#define BGFX_CHUNK_MAGIC_PRI BX_MAKEFOURCC('P', 'R', 'I', 0x0)
		...
		bx::ReallocatorI* allocator = s_allocator;
		...
		while (4 == bx::read(_reader, chunk) )
		{
			...
			default:
				CA_ERROR("%08x at %d", chunk, bx::skip(_reader, 0) );
				break;
			}
		}
	}
...
{
	MeshState* state = (MeshState*)BX_ALLOC(s_allocator, sizeof(MeshState) );
	return state;
}

void meshStateDestroy(MeshState* _meshState)
{
	BX_FREE(s_allocator, _meshState);
}
...
{
	char sBuffer[8192];
	vsprintf(sBuffer, _format, _argList);
	CA_TRACE(sBuffer);
}

void CallbackStub::fatal(Fatal::Enum _code, const char* _str)
{
	if (bgfx::Fatal::Enum::DebugCheck == _code)
	{
		bx::debugBreak();
	}
	else
	{
		CA_FATAL("0x%08x: %s", _code, _str);
		BX_UNUSED(_code, _str);
		abort();
	}
}

uint32_t CallbackStub::cacheReadSize(uint64_t /*_id*/)
{
	return 0;
}

bool CallbackStub::cacheRead(uint64_t /*_id*/, void* /*_data*/, uint32_t /*_size*/)
{
	return false;
}

void CallbackStub::cacheWrite(uint64_t /*_id*/, const void* /*_data*/, uint32_t /*_size*/)
{
}
...
void CallbackStub::captureBegin(uint32_t /*_width*/, uint32_t /*_height*/, uint32_t /*_pitch*/, TextureFormat::Enum /*_format*/, bool /*_yflip*/)
{
	BX_TRACE("Warning: using capture without callback (a.k.a. pointless).");
}

void CallbackStub::captureFrame(const void* /*_data*/, uint32_t /*_size*/)
{
}

void CallbackStub::captureEnd()
{
}

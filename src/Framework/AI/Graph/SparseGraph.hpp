...
#include  "NodeTypeEnumerations.hpp"
...
namespace CasaEngine
{
	...
	public:
		...
		int   NumEdges()const
		{
			...
			for (typename EdgeListVector::iterator curEdge = m_Edges.begin();
				curEdge != m_Edges.end();
				++curEdge)
			{
				tot += curEdge->size();
	...
	{
		CA_ASSERT(from < m_Nodes.size() && from >= 0 && m_Nodes[from].Index() != invalid_node_index,
			"<SparseGraph::GetEdge>: invalid 'from' index");

		CA_ASSERT(to < m_Nodes.size() && to >= 0 && m_Nodes[to].Index() != invalid_node_index,
		...
		CA_ASSERT("<SparseGraph::GetEdge>: edge does not exist", 0);
	}
	...
	{
		CA_ASSERT("<SparseGraph::GetEdge>: invalid 'from' index",
		...
		CA_ASSERT(0, "<SparseGraph::GetEdge>: edge does not exist");
	}
	...
	void SparseGraph<node_type, edge_type>::AddEdge(EdgeType edge)
	{
		...
		CA_ASSERT(edge.From() < m_iNextNodeIndex && edge.To() < m_iNextNodeIndex,
	...
	int SparseGraph<node_type, edge_type>::AddNode(node_type node)
	{
		if (node.Index() < static_cast<int>(m_Nodes.size()))
		{
			...
			CA_ASSERT(m_Nodes[node.Index()].Index() == invalid_node_index,
		...
		CA_ASSERT(node.Index() == m_iNextNodeIndex,
  ...
  //------------------------------------------------------------------------
	...
	bool SparseGraph<node_type, edge_type>::Save(std::ofstream& stream)const
	{
		...
		typename NodeVector::iterator curNode = m_Nodes.begin();
		...
		{
			for (typename EdgeList::iterator curEdge = m_Edges[nodeIdx].begin();

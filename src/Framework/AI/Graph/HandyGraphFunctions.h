#ifndef GRAPH_FUNCS_H_
#define GRAPH_FUNCS_H_
//-----------------------------------------------------------------------------
//
  ...
  for (int i=-1; i<2; ++i)
  {
    for (int j=-1; j<2; ++j)
    {
      ...
      if (ValidNeighbour(nodeX, nodeY, NumCellsX, NumCellsY))
      {
        ...
        Vector2F PosNode      = graph.GetNode(row*NumCellsX+col).Pos();
        Vector2F PosNeighbour = graph.GetNode(nodeY*NumCellsX+nodeX).Pos();
...
void WeightNavGraphNodeEdges(graph_type& graph, int node, float weight)
{
  ...
  CA_ASSERT(node < graph.NumNodes(), "WeightNavGraphNodeEdges()");
...
std::vector<std::vector<float> > CreateAllPairsCostsTable(const graph_type& G)
{
  ...
  std::vector<float> row(G.NumNodes(), 0.0);
  std::vector<std::vector<float> > PathCosts(G.NumNodes(), row);

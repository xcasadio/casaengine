...
void ArcBallCameraController::ReInit()
{
	...
	m_ArcBallOrientation.FromAxisAngle(Vector3F::Up(), MATH_PI);
	m_Target = Vector3F::Zero();
	m_fDistance = 5.0f;
	m_fArcBallYaw = MATH_PI;
...
Vector3F ArcBallCameraController::Target() const { return m_Target; }
void ArcBallCameraController::Target(CasaEngine::Vector3F val) 
{ 
	m_bRecomputeViewMatrix = true;
	m_Target = val; 
}
float ArcBallCameraController::Distance() const { return m_fDistance; }
void ArcBallCameraController::Distance(float val) 
{ 
	m_fDistance = val; 
	m_bRecomputeViewMatrix = true;
}
float ArcBallCameraController::InputDistanceRate() const { return m_fInputDistanceRate; }
void ArcBallCameraController::InputDistanceRate(float val) { m_fInputDistanceRate = val; }
